...
try:
    from live_assist.clients import model_factory as _mf  # noqa: F401 — triggers dotenv load
except Exception:
    pass

log = logging.getLogger(__name__)

from live_assist.rag_pipeline.paths import PROJECT_ROOT

_raw_persist_dir = os.environ.get(
    "CHROMA_PERSIST_DIR",
    str(PROJECT_ROOT / "runtime" / "chroma_db"),
)

if not Path(_raw_persist_dir).is_absolute():
    CHROMA_PERSIST_DIR = str(PROJECT_ROOT / _raw_persist_dir)
else:
    CHROMA_PERSIST_DIR = _raw_persist_dir
...
class ChromaChunkStore:
    ...
    def index_chunks(
        ...
        for c, vec in zip(chunks, vectors):
            ...
            documents.append(c.get("text_with_context") or c.get("text") or c.get("content") or "")
            metadatas.append({
                "chunk_id":          c["chunk_id"],
    ...
    def _build_where(self, filters: dict | None) -> dict | None:
        if not filters:
            return None
        clauses = []
        for key in ("doc_id", "source_filename", "chunking_strategy", "user_id"):
            if filters.get(key):
