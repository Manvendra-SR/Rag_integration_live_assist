...
def _invoke_turn_workflow(
    ...
    *,
    ...
    # Create Langfuse handler for observability (Requirements 2.1, 2.2, 6.1, 6.2, 6.3)
    langfuse_handler = create_langfuse_handler(
        session_id=session_id,
        user_id=settings.live_feedback_user_id,
        trace_name=f"RAG Query: {text[:50]}",
        metadata={
    ...
    # Log trace URL for verification (Requirement 11.1)
    if langfuse_handler:
        trace_url = get_trace_url(session_id)
        debug_log(f"[Langfuse] Trace URL: {trace_url}")
    
    try:
        # Get base workflow config
        config = _workflow_config(session_id)
        
        # Merge callbacks into config if handler was created (Requirement 2.2, 10.1, 10.2)
        if langfuse_handler:
            ...
            config["run_name"] = f"RAG Query: {text[:50]}"
        ...
        # Flush handler to ensure traces are sent (Requirement 2.5, 6.5)
        if langfuse_handler:
            try:
                if hasattr(langfuse_handler, "flush"):
                    langfuse_handler.flush()
                else:
                    # In Langfuse 4.x, flush is handled globally or on the client
                    from langfuse import Langfuse
                    Langfuse().flush()
            except Exception as e:
                debug_log(f"[Langfuse] Failed to flush handler: {e}")
...
async def _run_live_assist_workflow(
    *,
    ...
    try:
        ...
        workflow_response["_cache_hit"] = workflow_response.get("cache_hit", False)
        workflow_response["_cache_similarity"] = workflow_response.get("cache_similarity")
        workflow_response["_cache_result"] = "HIT" if workflow_response.get("cache_hit") else "MISS"
        ...
        metadata = {
            ...
            "cache_hit": workflow_response.get("_cache_hit", workflow_response.get("cache_hit", False)),
            "cache_similarity": workflow_response.get("_cache_similarity", workflow_response.get("cache_similarity")),
        }
